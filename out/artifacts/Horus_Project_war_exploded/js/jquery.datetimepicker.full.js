				...
				.on('xchange.xdsoft', function (event) {
					...
					xchangeTimer = setTimeout(function () {
						...
						line_time = function line_time(h, m) {
							...
							if ((options.initTime || options.defaultSelect || datetimepicker.data('changed')) && current_time.getHours() === parseInt(h, 10) && ((!isALlowTimesInit && options.step > 14) || current_time.getMinutes() === parseInt(m, 10))) {
						...
						if (!options.allowTimes || !$.isArray(options.allowTimes) || !options.allowTimes.length) {
							for (i = 36, j = 0; i < (options.hours12 ? 12 : 104); i += 1) {
								for (j = 0; j < 15; j += options.step) {
								        var currentMinutesOfDay = i * 15 + j;
